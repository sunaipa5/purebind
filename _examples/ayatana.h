 ...
 *
 * Checks to see if @klass is in the object class hierarchy of
 * #AppIndicatorClass.
 */
/**
...
#define APP_INDICATOR(obj)                                                     \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), APP_INDICATOR_TYPE, AppIndicator))
#define APP_INDICATOR_CLASS(klass)                                             \
  (G_TYPE_CHECK_CLASS_CAST((klass), APP_INDICATOR_TYPE, AppIndicatorClass))
#define APP_IS_INDICATOR(obj)                                                  \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), APP_INDICATOR_TYPE))
#define APP_IS_INDICATOR_CLASS(klass)                                          \
  (G_TYPE_CHECK_CLASS_TYPE((klass), APP_INDICATOR_TYPE))
#define APP_INDICATOR_GET_CLASS(obj)                                           \
  (G_TYPE_INSTANCE_GET_CLASS((obj), APP_INDICATOR_TYPE, AppIndicatorClass))
 ...
 * @APP_INDICATOR_CATEGORY_APPLICATION_STATUS: The indicator is used to display
 * the status of the application.
 * @APP_INDICATOR_CATEGORY_COMMUNICATIONS: The application is used for
 * communication with other people.
 * @APP_INDICATOR_CATEGORY_SYSTEM_SERVICES: A system indicator relating to
 * something in the user's system.
 * @APP_INDICATOR_CATEGORY_HARDWARE: An indicator relating to the user's
 * hardware.
 * @APP_INDICATOR_CATEGORY_OTHER: Something not defined in this enum, please
 ...
 * @APP_INDICATOR_STATUS_ACTIVE: The indicator should be shown in it's default
 * state.
 * @APP_INDICATOR_STATUS_ATTENTION: The indicator should show it's attention
 * icon.
 *
...
struct _AppIndicatorClass {
  ...
  void (*new_status)(AppIndicator *indicator, const gchar *status,
                     gpointer user_data);
  void (*new_icon_theme_path)(AppIndicator *indicator,
                              const gchar *icon_theme_path, gpointer user_data);
  void (*new_label)(AppIndicator *indicator, const gchar *label,
                    const gchar *guide, gpointer user_data);

  /* Local Signals */
  void (*connection_changed)(AppIndicator *indicator, gboolean connected,
                             gpointer user_data);

  void (*scroll_event)(AppIndicator *indicator, gint delta,
  ...
  void (*unfallback)(AppIndicator *indicator, GtkStatusIcon *status_icon);
...
AppIndicator *
app_indicator_new(const gchar *id, const gchar *icon_name,
                  AppIndicatorCategory category) G_GNUC_DEPRECATED;
AppIndicator *
app_indicator_new_with_path(const gchar *id, const gchar *icon_name,
...
void app_indicator_set_attention_icon(AppIndicator *self,
    ...
    G_GNUC_DEPRECATED_FOR(app_indicator_set_attention_icon_full);
...
void app_indicator_set_icon(AppIndicator *self, const gchar *icon_name)
    G_GNUC_DEPRECATED_FOR(app_indicator_set_icon_full);
